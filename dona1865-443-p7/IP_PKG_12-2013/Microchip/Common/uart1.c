...
Microchip micro controllers or Microchip digital signal controllers; provided
...
//U1BRG register value and baud rate mistake calculation

#if defined (__PIC32MX__)
    #define BAUDRATEREG1        ((GetPeripheralClock()+(BRG_DIV2/2*BAUDRATE1))/BRG_DIV2/BAUDRATE1-1)
#else
...
Precondition:None.
Overview:    This routine checks the UART baud rate error percentage and returns it.
Input: None.
...
Precondition: UART1Init must be called prior to calling this
              routine.
Overview: This routine waits for a byte to be received.  
...
Function: UART2Init()
Precondition: None.
Overview: This routine sets up the UART2 module.
Input: None.
Output: None.
Notes:
